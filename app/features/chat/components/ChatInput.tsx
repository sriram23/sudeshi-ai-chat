...
const ChatInput = ({ input, setInput, settings, setSettings, status, sendMessage, stopStreaming }:{ input: string; setInput: (value: string) => void; settings: { model: string }; setSettings: (newSettings: { model: string }) => void; status: string; sendMessage: (value: string, model: string) => void; stopStreaming: () => void }) => {
    ...
    return (
        <div className="w-full flex flex-col relative">
            ...
            <div className="absolute right-2 bottom-1">
                <select onChange={(e) => setSettings({ model: e.target.value })} value={settings?.model} className="mr-2 p-1 border rounded">
