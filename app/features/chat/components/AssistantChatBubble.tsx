...
const AssistantChatBubble = memo(
  ({
    message,
    thinking,
    currentResponse,
        ...
        <div className="flex items-center text-gray-900 dark:text-gray-100 px-2 m-1 rounded-lg">
          {currentThinking && !currentResponse && (
            <div className="p-2 pl-4 py-3 my-1 rounded self-start max-w-3xl bg-zinc-100 dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800">
              <div className="mb-2 text-[10px] uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">
                Thinking
              </div>
    ...
    transition-colors"
            ...
            {usage && (
              <button
                title="Metrics"
                aria-label="Metrics"
                className={`${showMetric ? 'bg-zinc-200 ' : ''}p-1.5 rounded-md
    text-zinc-400
    hover:bg-zinc-100 hover:text-zinc-700
    dark:hover:bg-zinc-800 dark:hover:text-zinc-200
    transition-colors`}
            ...
            <div
              title={
                msgStatus ? (contextThresholdExceeded ? 'High token usage' : msgStatus) : undefined
              }
              className="
                            inline-flex
                            items-center
        ...
        {usage && showMetric && (
          <div className="flex flex-col m-4 max-w-xl">
            <MetricsCard
              ...
              promptTokenCost={usage?.prompt_cost ?? 0}
              completionTokenCost={usage?.completion_cost ?? 0}
              tokenSpeed={metrics?.tokensPerSecond}
              firstChunk={metrics?.timeToFirstChunk}
              streaming={metrics?.streamingTime}
            />
          </div>
        )}
        <Alert
          className={`fixed max-w-md top-4 right-4 z-50 ${alertMessage.type === 'destructive' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'} ${showAlert ? 'block' : 'hidden'}`}
        >
          <AlertTitle>
            <span className="flex items-center gap-2">
              <Info size={16} /> {alertMessage.text}
            </span>
          </AlertTitle>
        </Alert>
      </div>
